...
impeach %>%
  filter(party == "D") %>%
  group_by(for_impeachment) %>%
  summarize(trump_percent = mean(trump_percent))
...
new_collumns <- impeach  %>%
mutate(year_announced = ifelse(for_impeachment == "YES", as.numeric(format(as.Date(date_announced), "%Y")), NA),
         month_announced = ifelse(for_impeachment == "YES", as.numeric(format(as.Date(date_announced), "%m")), NA))
